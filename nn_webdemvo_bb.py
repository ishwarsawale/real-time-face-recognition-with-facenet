...
def recom():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # id_dataset = id_data.get_id_data('./base_dir', pnet, rnet, onet, sess, embeddings, images_placeholder, phase_train_placeholder)
            # with open('weights', 'wb') as fp:
                # pickle.dump(id_dataset, fp)
            with open ('weights', 'rb') as fp:
                id_dataset = pickle.load(fp)
            # print(type(id_dataset))
            # twoFace.print_id_dataset_table(id_dataset)
            ...
            while True:
                ...
                if nrof_faces >= 1:
                    ...
                    for i in range(nrof_faces):
                        ...
                        # print('basic distance image to image for %s person' % matching_id)
                        ...
                        for H_i in HumanNames:
                            ...
                            if HumanNames[best_class_indices[0]] == H_i:
                                ...
                                conf = (2 - dist) * 50 
                                dist_threshold = 0.3
                                if (dist >= dist_threshold) and (result_names == matching_id) and (best_class_probabilities >= 0.90) and (conf > 60):
                                    ...
                                    print('basic distance image to image for %s person' % matching_id)
                                    print(matching_id, dist)
                                    print('probability score from model: ', best_class_probabilities)
                                    if show_id:
                                        font = cv2.FONT_HERSHEY_SIMPLEX
                                        cv2.putText(frame, result_names, (text_x, text_y), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
